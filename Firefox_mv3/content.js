(async function() {
  ...
  if (!document.hidden && window.focus) connectToBackground();
  if (Options.blackList.split(/[,;]/g).some(e => !!e.trim() ? location.host.includes(e.trim()) : false)) return;
  if (window !== window.top) return;

  function requestTranslateBatch(textArray) {
    return new Promise(res => {
      browser.runtime.sendMessage({ action: "translateBatch", texts: textArray, options: Options }, (response) => {
        res(response ? response.translatedTexts : textArray);
      });
    });
  }
  ...
  const observer = new MutationObserver(async (mL) => {
    ...
    if (nodesToTranslate.length > 0 && true) {
  ...
  function collectTextNodes(root, bucket) {
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode: (node) => {
        ...
        if (skipTags.includes(parentTag) || !node.nodeValue.trim()) return NodeFilter.FILTER_REJECT;
  ...
  async function applyTranslation() {
    browser.runtime.sendMessage({ action: "translate", text: originalTitle, options: Options }, (response) => {
  ...
  function prepareUI() {
    ...
    shadow.getElementById(`optionsBtn`)?.addEventListener("click", () => {
	  browser.runtime.sendMessage( { action: "openOptionsPage"}, (response) => {console.log(response)} );
    });
  }
  ...
  function connectToBackground() {
    ...
    try {
      port = browser.runtime.connect({ name: 'Long_live_me' });
      console.log('[Content] Tab Active: Đã kết nối Port');
      ...
      port.onDisconnect.addListener(() => {
        console.log('[Content] Port bị đóng.');
        cleanUpPort();
        if (!document.hidden && window.focus) {
          reconnectTimeout = setTimeout(connectToBackground, 500);
        }
      });
        ...
        if (port) {
          try {
            ...
            pongTimeout = setTimeout(() => {
              console.warn('[Content] Không nhận được PONG. Reset kết nối...');
              cleanUpPort();
              connectToBackground();
            }, 5000);
      ...
      reconnectTimeout = setTimeout(() => {
        console.log('[Content] Tự động xoay vòng Port để reset timer 5 phút.');
        cleanUpPort();
        connectToBackground();
      }, 270000);

    } catch (err) {
      console.warn('[Content] Không thể thiết lập kết nối:', err);
    }
  }
  ...
  function cleanUpPort() {
    ...
    if (port) {
      try { port.disconnect(); } 
	  catch(e) {console.log(browser.runtime.lastError)}
      port = null;
      console.log('[Content] Tab Inactive: Đã ngắt Port');
    }
  }

  document.addEventListener('visibilitychange', () => {
    if (document.hidden || !window.focus) cleanUpPort();
    else connectToBackground();
  });
