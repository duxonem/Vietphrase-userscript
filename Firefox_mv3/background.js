  ...
  switch(message.action) {
	  ...
	  case 'addDict':
	    (async () => {
		  ...
		  const textContent =  await message.file.text()
		  const dict = importDict(textContent);
		  if (!currentDicts) currentDicts= Object.create(null);
		  currentDicts[name] = { dict, priority };            
		  sendResponse({ status: "success" });
		  return true;
		})();
		break;
  ...
  if (port.name === 'Long_live_me') { 
    ...
    port.onMessage.addListener((message) => {
      if (message.action === 'ping') { 
        ...
        browser.storage.local.get(['__keepAlive']);
      }
    });
...
(async function init() {	
  await preloadData();
  currentDicts = currentDicts ?? (await dbGet("dictsCache") || Object.create(null));
})();