(async function() {
  ...
  function requestTranslateBatch(textArray) {
    return new Promise(res => {
      chrome.runtime.sendMessage({ action: "translateBatch", texts: textArray, options: Options }, (response) => {
  ...
  async function applyTranslation() {
    chrome.runtime.sendMessage({ action: "translate", text: originalTitle, options: Options }, (response) => {
  ...
  function prepareUI() {
    ...
    shadow.getElementById(`optionsBtn`)?.addEventListener("click", () => {
	  chrome.runtime.sendMessage( { action: "openOptionsPage"}, (response) => {console.log(response)} );
    });
  }
  ...
  function connectToBackground() {
    ...
    try {
      port = chrome.runtime.connect({ name: 'Long_live_me' });
  ...
  function cleanUpPort() {
    ...
    if (port) {
      try { port.disconnect(); } 
	  catch(e) {console.log(chrome.runtime.lastError)}
      port = null;
